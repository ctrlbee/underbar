(function() {
  ...
  _.last = function(array, n) {
    var end = array.length<n ? 0 : array.length-n;
    return n === undefined ? array[array.length-1] : array.slice(end); 
  };
  ...
  _.indexOf = function(array, target){
    ...
    _.each(array, function(item, index) {
      if(item===target && array[result]!==item)
        result = index; 
    });
  ...
  _.extend = function(obj) {
    _.each(arguments, function(addObj){
  ...
  _.shuffle = function(array) {
    ...
    var random = Math.floor(Math.random()*10/array.length); 
    console.log(random);
